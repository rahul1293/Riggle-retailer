        ...
        dataManager.verifyOtp(object : ApiResponseListener<APICommonResponse<UserData>> {
            override fun onSuccess(response: APICommonResponse<UserData>) {
                ...
                response.user?.let {
                    ...
                    if (response.user != null && (response.user?.retailer?.account_status.equals("completed") || (response.user?.retailer?.store_type!=null && !response.user?.retailer?.store_type.equals("")))/*response.user?.retailer?.account_status.equals("completed")*/) {
                        userPreference
